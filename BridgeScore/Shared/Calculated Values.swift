...
enum CalculatedFunction : String, CaseIterable, Codable {
    ...
    var argumentLimits: (min: Int?, max: Int?) {
        ...
        case .abs, .formatMade:
            return (min: 1, max: 1)
        case .mod, .match, .safeDivide, .round, .toString:
            return (min: 2, max: 2)
    ...
    var type: CalculatedType {
        switch self {
        case .lowercased, .uppercased, .capitalized, .formatMade, .toString:
                .string
        case .match:
                .boolean
        default:
...
indirect enum CalculatedParseNode : Equatable {
    ...
    public func value<ViewModel>(viewModel: ViewModel, variableValue: (InsightColumn, ViewModel) throws ->CalculatedValue?) throws -> CalculatedValue {
        ...
        case .function(function: let function, arguments: let arguments):
            ...
            case .round:
                return CalculatedValue(Utility.round(values[0].numeric!, places: Int(values[0].numeric!)))
            case .toString:
                return CalculatedValue(values[0].numeric!.toString(places: Int(values[1].numeric!)))
            case .formatMade:
