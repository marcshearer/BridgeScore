//
//  Input Focused.swift
//  BridgeScore
//
//  Created by Marc Shearer on 06/04/2026.
//

import SwiftUI

struct InputFocused<FocusType> : View where FocusType: Hashable {
    
#if canImport(UIKit)
    typealias KeyboardType = UIKeyboardType
#else
    enum KeyboardType {
        case `default`
        case URL
    }
#endif
    
#if canImport(UIKit)
    typealias CapitalizationType = UITextAutocapitalizationType
#else
    enum CapitalizationType {
        case sentences
        case none
    }
#endif
    
    var title: String?
    var field: Binding<String>
    @FocusState.Binding var focusedField: FocusType?
    var focusValue: FocusType?
    var message: Binding<String>?
    var placeHolder: String? = nil
    var topSpace: CGFloat = 0
    var leadingSpace: CGFloat = 0
    var height: CGFloat = 40
    var width: CGFloat?
    var color: PaletteColor = Palette.input
    var font: Font = inputFont
    var keyboardType: KeyboardType = .default
    var autoCapitalize: CapitalizationType = .sentences
    var autoCorrect: Bool = true
    var multiLine: Bool = false
    var clearText: Bool = true
    var asText: Bool = false
    var inlineTitle: Bool = true
    var inlineTitleWidth: CGFloat = 180
    var accessibilityIdentifier: String = ""
    var onLoseFocus: (()->())? = nil
    var onChange: ((String)->())? = nil
    @State private var refresh = false
    
    var body: some View {
        
        VStack(spacing: 0) {
            // Just to trigger view refresh
            if refresh { EmptyView() }
            
            if title != nil && !inlineTitle {
                HStack {
                    InputTitle(title: title, message: message, topSpace: topSpace, width: (width == nil ? nil : width! + leadingSpace + 16 + (clearText && !asText ? 20 : 0)))
                    Spacer().frame(width: clearText && !asText ? 20 : 0)
                }
                .frame(height: 22)
                Spacer().frame(height: 8)
            } else if let message = message?.wrappedValue {
                HStack {
                    Spacer()
                    Text(message)
                        .foregroundColor(Palette.background.strongText)
                        .font(messageFont)
                    Spacer().frame(width: 16)
                }
                .frame(height: 8)
            } else {
                Spacer().frame(height: topSpace)
            }
            HStack(spacing: 0) {
                Spacer().frame(width: leadingSpace)
                if title != nil && inlineTitle {
                    HStack {
                        Spacer().frame(width: 8)
                        VStack(spacing: 0) {
                            Spacer()
                            Text(title!)
                                .frame(height: height - 8)
                            Spacer()
                        }
                        Spacer()
                    }
                    .frame(width: inlineTitleWidth)
                    .frame(height: height).layoutPriority(999)
                    Spacer().frame(width: 14)
                }
                HStack {
                    ZStack {
                        if !asText && field.wrappedValue == "" {
                            if let placeHolder = placeHolder {
                                VStack(spacing: 0) {
                                    HStack {
                                        Spacer().frame(width: 5)
                                        Text(placeHolder)
                                            .accessibilityIdentifier(accessibilityIdentifier + " TextField for placeHolder")
                                            .foregroundColor(Palette.input.faintText.opacity(0.5))
                                        Spacer()
                                    }
                                    .frame(height: height)
                                }
                                .frame(height: height).layoutPriority(999)
                            }
                        }
                        VStack(spacing: 0) {
                            Spacer().frame(height: 4)
                            if asText {
                                HStack {
                                    Spacer().frame(width: 6)
                                    Text(field.wrappedValue)
                                        .accessibilityIdentifier(accessibilityIdentifier + " Text for asText")
                                        .truncationMode(.tail)
                                        .foregroundColor(color.text)
                                        .if(multiLine) { view in
                                            view.lineLimit(1)
                                        }
                                }
                            } else if multiLine && height > inputDefaultHeight {
                                UndoWrapper(field) { field in
                                    TextEditor(text: field)
                                        .focusEffectDisabled()
                                        .accessibilityIdentifier(accessibilityIdentifier + " TextEditor")
                                        .scrollContentBackground(.hidden)
                                        .frame(height: height - 4)
                                        .focused($focusedField, equals: focusValue)
                                        .if(!multiLine) { view in
                                            view.lineLimit(1)
                                        }
                                        .keyboardType(self.keyboardType)
                                        .autocapitalization(autoCapitalize)
                                        .disableAutocorrection(!autoCorrect)
                                        .foregroundColor(color.text)
                                        .onChange(of: field.wrappedValue, initial: false) {
                                            for special in ["\n", "\t"] {
                                                if field.wrappedValue.contains(special) {
                                                    field.wrappedValue = field.wrappedValue.replacingOccurrences(of: special, with: "")
                                                    focusedField = nil
                                                    onLoseFocus?()
                                                }
                                            }
                                            onChange?(field.wrappedValue)
                                        }
                                        .scrollIndicators(multiLine ? .visible : .hidden)
                                }
                            } else {
                                UndoWrapper(field) { field in
                                    TextField("", text: field)
                                        .focusEffectDisabled()
                                        .accessibilityIdentifier(accessibilityIdentifier + " TextField")
                                        .onKeyPress(keys: [.tab, .return]) { key in
                                            focusedField = nil
                                            Utility.executeAfter(delay: 0.1) {
                                                onLoseFocus?()
                                            }
                                            return .handled
                                        }
                                        .scrollContentBackground(.hidden)
                                        .frame(height: height - 4)
                                        .focused($focusedField, equals: focusValue)
                                        .if(!multiLine) { view in
                                            view.lineLimit(1)
                                        }
                                        .keyboardType(self.keyboardType)
                                        .autocapitalization(autoCapitalize)
                                        .disableAutocorrection(!autoCorrect)
                                        .foregroundColor(color.text)
                                        .onChange(of: field.wrappedValue, initial: false) {
                                            onChange?(field.wrappedValue)
                                        }
                                        .scrollIndicators(multiLine ? .visible : .hidden)
                                }
                            }
                        }
                        .frame(height: height).layoutPriority(999)
                    }
                }
                .background(color.background)
                .if(width != nil) { (view) in
                    view.frame(width: width)
                }

                if width == nil {
                    Spacer()
                }
                
                if clearText && !asText {
                    VStack {
                        Spacer()
                        Button {
                            field.wrappedValue = ""
                        } label: {
                            Image(systemName: "x.circle.fill").font(inputTitleFont).foregroundColor(Palette.clearText)
                        }
                        Spacer()
                    }.frame(width: 20)
                }
                
                Spacer().frame(width: 16)
            }
            
            .font(font)
        }
        .frame(height: self.height + self.topSpace + (title != nil && !inlineTitle ? 24 : (message != nil ? 16 : 0)))
        .if(width != nil) { (view) in
            view.frame(width: width! + leadingSpace + 16 + (clearText && !asText ? 20 : 0))
        }
    }
}
