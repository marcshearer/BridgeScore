...
let analysisBannerHeight: CGFloat = 90
let analysisBannerBottom: CGFloat = 6
let analysisCornerSize: CGFloat = 8
    ...
    var body: some View {
        GeometryReader { viewGeometry in
            HStack {
                ...
                ZStack {
                    VStack(spacing: 0) {
                        Spacer().frame(height: analysisBannerHeight + analysisBannerBottom)
            ...
            VStack(spacing: 0) {
                ...
                ZStack {
                    VStack(spacing: 0) {
                        ...
                        .frame(height: analysisBannerHeight)
                        .ignoresSafeArea()
                        Spacer()
                    }
                    VStack(spacing: 0) {
                        ...
                        HStack {
                            ...
                            VStack(spacing: 0) {
                                if traveller.rankingNumber == handTraveller.rankingNumber {
                                    HStack {
                                        Spacer().frame(width: 20)
                                        AnalysisCommentView(board: $board, stopEdit: $stopEdit, color: .windowBannerShadow)
                                            .cornerRadius(analysisCornerSize)
                                        Spacer().frame(width: 20)
                                    }
                                } else {
            ...
            VStack {
                LazyVGrid(columns: columns, spacing: 0) {
                    GridRow {
                        ...
                        let travellerIndex = traveller.boardIndex
                        CenteredText("Bidding \(boardIndex) \(travellerIndex)")
                        Text("")
                        CenteredText("Play")
                    }
                    .bold()
        ...
        ZStack {/*
            VStack {
                ...
                RoundedRectangle(cornerRadius: analysisCornerSize).foregroundColor(PaletteColor(color).background)
                    .frame(height: analysisBannerHeight - 12)
                Spacer()
            }
            HStack {
                ...
                ZStack {
                            ...
                            VStack(spacing: 0) {
                                ...
                                HStack {
                                    ...
                                    GeometryReader { geometry in
                                        let frame = CGRect(x: geometry.frame(in: .global).minX, y: geometry.frame(in: .global).minY, width: geometry.size.width, height: analysisBannerHeight - 24)
                                        TextViewWrapper(frame: frame, field: $comment, focused: $focused)
                                            .paletteColor(Palette.windowBannerShadow)
                                            .onChange(of: $comment.wrappedValue, initial: false) {
                                                if comment.contains("\n") || comment.contains("\t") {
                                ...
                                VStack {
                                    VStack {
                                        ...
                                        Button {
                                            ...
                                            Image(systemName: "x.circle.fill").font(inputTitleFont).foregroundColor(Palette.bannerInput.background)
                                        }
                                        Spacer()
                                    }
                                    .frame(width: 40, height: analysisBannerHeight - 16)
                                    Spacer()
                                }
                    ...
                    VStack {
                        ...
                        .frame(height: analysisBannerHeight - 16)
                        Spacer()
                    }
                }
            }
